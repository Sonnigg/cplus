...
static void usage(void)
{
    ...
    printf(
        ...
        "  -D <macro>    Define preprocessor macro (forwarded to Backend)\n"
        "  -I <dir>      Add include directory (forwarded to Backend)\n"
        "  -shared       Create a shared library/dll (forwarded to Backend)\n"
        "  -ar <lib>     Create a static library (forwarded to TCC)\n"
        "  -impdef <lib> Create a def file for the dynamic library (forwarded to TCC)\n"
        "  -nostdlib     Do not link with standard CRT and libraries (forwarded to Backend)\n"
        "  -nostdinc     Do not use standard system include paths (forwarded to Backend)\n"
        "  -m32/64       Defer to i386/x86_64 (x86/x64) cross compiler (forwarded to Backend)\n"
        "  -g            Generate runtime debug info (forwarded to Backend)\n"
        "  -b            Compile with built-in memory and bounds checker (implies -g, forwarded to TCC)\n"
        "  -L<dir>       Add library path 'dir' (forwarded to Backend)\n"
        "  -l<lib>       Link with dynamic or static library 'lib' (forwarded to Backend)\n"
        "  --keep-c      Keep generated .gencx.c file\n"
        "  --index       Emit token index for IDE highlighting services\n"
        "  --            Stop processing C+ options\n\n"
        "Info about Backends:\n"
        "  C+ can target Clang and GCC via the --release flag and targets TinyCC by default.\n"
        "  Only -impdef <lib>, -ar <lib> and -b are flags that cannot be used with --release as they are TCC-specific.\n",
        cplus_version,
        cplus_version
    );
}
